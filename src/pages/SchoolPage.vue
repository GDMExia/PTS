<template>
    <div style="height:100%;">
        <div>
            <confirm v-model="show" title="有验证码才可查看哦" confirm-text="确定" cancel-text="返回" @on-cancel="onCancel" @on-confirm="onConfirm"
                :show-input="true" :placeholder="'请输入验证码'" :content="'有验证码才可查看哦'" :close-on-confirm="false" @on-blur="fixScroll">
            </confirm>
        </div>
        ...
        <div class="menu" :style="scrollBoxStyle" ref="scrollBox" @scroll="menuScroll">
            ...
            <div class="loading" v-show="loading">
                <img src="../assets/loading.png" class="loading-icon" alt="">
                <span>正在加载</span>
            </div>
                ...
                this.$http.get(`${this.rootPath}/Index/getArticle?page=${this.page}&pageSize=${this.pageSize}&order_sort=${this.btn}`).then(res => {
                    if (res.data.StatusInfo.success) {
                        ...
                        if (this.page > res.data.PageInfo.TotalPages) {
                            ...
                            this.articleList = [...this.articleList, ...res.data.articleList]
...
<style scoped>
    * {
    ...
    .header_btn:first-child div {
        width: 52px;
        height: 87px;
        background: #FFF url('../../static/img/icon/ic_rmwz@2x.png') top no-repeat;
    ...
    .header_btn:nth-child(2) div {
        width: 52px;
        height: 87px;
        background: #FFF url('../../static/img/icon/ic_ysd@2x.png') top no-repeat;
        background-size: 52px 52px;
        text-align: center;
        color: #fff;
        padding-top: 56px;
    ...
    .header_btn:last-child div {
        width: 52px;
        height: 87px;
        background: #FFF url('../../static/img/icon/ic_tccx@2x.png') top no-repeat;
        background-size: 52px 52px;
        text-align: center;
        color: #fff;
        padding-top: 56px;
