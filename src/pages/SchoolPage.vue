<template>
	<div style="height:100%;">
		...
		<div ref="scrollHeader">
			...
			<div
				class="main"
				style="background-color:#fff;padding:19px 0 12px 0"
			>
				<Flexbox>
					<FlexboxItem
						class="header_btn"
            ...
            <div v-else @click="goToCause(item.cid)">
              <p>{{item.cate_name}}</p>
				...
				<div class="searchbar">
					...
					/>
					<img
						src="../../static/img/icon_search@2x.png"
...
export default {
	...
	methods: {
		...
		getSchoolList() {
			this.$http
				.get(`${this.rootPath}/Index/getArticleTree`)
				.then(res => {
					console.log(res);
		...
		goToArticleDetail(val) {
			this.$router.push({
				path: "/schools/detail",
				query: { article_id: val.article_id }
			});
		},
      ...
      goToCause(val){
		...
		confirmcode() {
			this.$http
				.get(
					`${this.rootPath}/Index/checkArticleCode?code_name=${this.code_name}&cid=${this.cid}`
				)
				.then(res => {
					console.log(res);
					if (res.data.StatusInfo.success) {
	...
	computed: {
		scrollBoxStyle() {
			return {
				height: this.winHeight - 197 - 56 - 37 + "px"
			};
		}
	},
	beforeDestroy() {},
