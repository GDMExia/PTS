<template>
	<div style="height:100%;">
        ...
        >
			<!-- <div
				style="display: flex;padding: 55% 0;justify-content: center;align-items: center;flex-direction:column;font-size: 16px;color: #ccc;"
			...
			> -->
            <div
                class="ofy_auto flx_1"
                ref="scrollBoxMain"
            >
                <div
                    class="container"
                    @click="handleDetail(item.tourism_id)"
...
export default {
	...
	data() {
		return {
			...
			list: [
				{
					id: 1,
					img: "http://iph.href.lu/355x177",
					title: "马来西亚、吉隆坡城市遗址、 洞穴与缆车马来西亚",
					created: "2019/07/24"
				},
				{
					id: 2,
	...
	methods: {
		...
		handleDetail(id) {
			if (
				this.userInfo.is_member == 0 ||
				this.userInfo.over_time < moment().format("YYYY-MM-DD")
			) {
				this.show = true;
			} else {
		...
		handleUser() {
			...
			this.userDetail(params).then(res => {
				if (res.StatusInfo.success) {
					...
					if (
						res.userInfo.is_member == 0 ||
						res.userInfo.over_time < moment().format("YYYY-MM-DD")
					) {
						this.show = true;
					}
				} else {
	...
	computed: {
        ...
        scrollBoxStyle() {
			return {
                height: this.winHeight - 95 - 56 + "px",
                overflowY:'auto'
			};
		}
    },
